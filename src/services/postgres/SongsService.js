...
class SongsService {
  ...
  async addSong({
    title, year, performer, genre, duration = null, albumId,
  }) {
    ...
    if (albumId) {
      albumById = await this._albumsService.getIdAlbum(albumId);
    }

    const query = {
      text: 'INSERT INTO songs VALUES($1, $2, $3, $4, $5, $6, $7) RETURNING id',
      values: [id, title, year, performer, genre, duration, albumById?.id || null],
    };
  ...
  async editSongById(id, {
    ...
    const query = {
      ...
      `,
      values: [title, year, performer, genre, duration, albumById?.id || null, id],
    };
